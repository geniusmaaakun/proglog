...
//複製のサーバを持つクラスタを設定し、Menbershipがメンバーシップに参加した全てのサーバーを返し、サーバーがクラスタから離脱した後に更新することを確認している
//joinsとleaveは、イベントがどこで何回起きたかを教えてくれる
...
//秋のポートで新たなメンバーを設定し、メンバーの長さをノード名にして、名前が一意になるようにする
//メンバーの長さはこのメンバーがクラスタの最初のメンバーなのか、それとも参加するクラスタがあるのかを教える
