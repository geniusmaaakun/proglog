...
func TestMultipleNodes(t *testing.T) {
	//3つのサーバーから構成されるクラスタを設定
	//raftのデフォルトのタイムアウトを短くし、Raftが素早くリーダーを選出できるようにしている
	...
	for i := 0; i < nodeCount; i++ {
		...
		//一つ目のサーバーはクラスタをブートストラップ(起動)してリーダーとなり
		//残る二つのサーバーをクラスタに追加している
		if i == 0 {
	...
	//リーダのサーバーにレコードを追加し、Raftがそのレコードをフォロワーに複製したことを確認することでレプリケーションのテストをする
	//Raftのフォロワーは短い待ち時間の後の追加メッセージを適用する
	records := []*api.Record{
		{Value: []byte("first")},
		{Value: []byte("second")},
	}
	...
	//リーダーがクラスタから離脱したサーバーへのレプリケーションを停止し、既存のサーバーへのレプリケーションを継続することの確認
	err := logs[0].Leave("1")
	require.NoError(t, err)
